...
species Merchant skills: [fipa, moving]{
	...
	reflex go_for_auction when: reachedauctioncentre = false {
		do goto target:{45,20} speed: 5.0;
		if(location = {45,20})
		{
	...
	reflex start_of_auction when: startofauction = true {
		...
		do start_conversation with: [ to :: [C1], protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['Auction is starting now.'] ];
		do start_conversation with: [ to :: [C2], protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['Auction is starting now.'] ];
		do start_conversation with: [ to :: [C3], protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['Auction is starting now.'] ];
		do start_conversation with: [ to :: [C4], protocol :: 'fipa-contract-net', performative :: 'inform', contents :: ['Auction is starting now.'] ];
		startofauction <- false;
	}
...
species Consumer0 skills: [fipa, moving]{
	...
	{
		if mad = 1 and updatepoint = true {
			baddypoint <- self.location;
		}
		do wander speed: 5.0;
		if mad = 1 and baddyalert = false and securityalert = false and endauction = true and wonauction = false {
			baddyalert <- true;
			updatepoint <- true;
		}
	}
	reflex gotcaught when: movebaddy = true and mad = 1 and endauction = true and wonauction = false
	{
		ask Security_Guard{
			myself.securitypoint <- self.location;
		}
		if(location distance_to(securitypoint) < 10)
		{
	...
	reflex auction_ended when: endauction = true and wonauction = true {
		write self.name + ': I have won. I am going home.';
		do goto target:{0,50} speed: 5.0;
		if(location distance_to({0,50}) < 1)
		{
			do die;
